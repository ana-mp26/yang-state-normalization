...
Information modeled using YANG and represented in a serialization format such as XML, JSON, or CBOR.

## Representation

A concrete serialized encoding of YANG state data.

Examples include XML, JSON, and CBOR encodings conveying the same underlying information.

## Normalized Representation

A deterministic representation generated by the normalization procedure defined in this document.

## Representation Equivalence

Two representations are representation-equivalent when they produce identical normalized representations.

## Structural Equivalence

The property whereby two representations convey the same observable structure and values after normalization.

## Fingerprint

A cryptographic digest calculated from a normalized representation.

## Serialization Format

The concrete encoding format used to represent YANG state data, including XML, JSON, and CBOR.

## Schema-Independent Normalization

A normalization procedure that does not require access to the corresponding YANG schema during processing.

## Semantic Equivalence

The property whereby two representations correspond to the same YANG state after considering allschema-dependent semantics.
